...
class SensorPlacementEnv:
    ...
    def __init__(self, candidates: gpd.GeoDataFrame, universe: gpd.GeoDataFrame,
        ...
        self.initial_solution = initial_solution if initial_solution is not None else []
        self.reset()
    ...
    def reset(self):
        ...
        # aplica baseline se existir
        if self.initial_solution:
            for idx in self.initial_solution:
                if idx < self.N:
                    self.selected.append(idx)
                    self.covered |= self.C[idx]
            self.t = len(self.selected)
